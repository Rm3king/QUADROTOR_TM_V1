 ...
 *
 * 使用约束：
 * 1. 本模块由调度器周期调用，不参与姿态解算与控制算法计算。
 * 2. 重构时不得修改协议帧格式、功能码、发送优先级和参数语义。
 * 3. 本文件允许做结构整理、注释统一和重复逻辑收敛，不允许改变外部行为。
 */
...
void ANO_DT_Send_Data(u8 *dataToSend , u8 length)
{
    ...
    UsbCdcSend( dataToSend , length );  /* USB 数据发送。 */
#endif
...
void ANO_DT_Data_Receive_Prepare(u8 data)
{
	...
	if(state==0&&data==0xAA)	/* 帧头起始字节。 */
	{
		state=1;
		DT_RxBuffer[0]=data;
	}
	else if(state==1&&data==0xAF)	/* 数据源，0xAF 表示上位机。 */
	{
		state=2;
		DT_RxBuffer[1]=data;
	}
	else if(state==2)		/* 目标地址。 */
	{
		state=3;
		DT_RxBuffer[2]=data;
	}
...
static void ANO_DT_Data_Receive_Anl_Task(void)
{
	...
	u8 len = UsbCdcRead(usbdatarxbuf,100);
	if(len)
	{
		for(u8 i=0; i<len; i++)
...
static void ANO_DT_Data_Receive_Anl(u8 *data_buf,u8 num)
{
	...
	for(u8 i=0;i<(num-1);i++)
		sum += *(data_buf+i);
	if(!(sum==*(data_buf+num-1)))		return;		/* 校验和错误。 */
	if(!(*(data_buf)==0xAA && *(data_buf+1)==0xAF))		return;		/* 帧头错误。 */
	
	if(*(data_buf+2)==MYHWADDR)
	{
		if(*(data_buf+3)==0XE0)			/* 命令帧。 */
		{
			switch(*(data_buf+5))		/* 功能字。 */
			{
				case 0x01:
...
void ANO_DT_Send_User()
{
    ...
    data_to_send[_cnt++]=0xf1; /* 用户自定义调试数据。 */
	data_to_send[_cnt++]=0;
	/* 当前自定义帧输出 6 组 OpenMV 相关调试量。 */
