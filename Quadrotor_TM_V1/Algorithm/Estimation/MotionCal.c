 ...
 *
 * 功能描述:
 *   运动数据融合模块，使用互补滤波器融合气压计/ToF 与加速度计数据：
 *   - wcz_acc_fus  -- 垂直加速度融合（积分修正滤波）
 *   - wcz_spe_fus  -- 垂直速度融合（微分修正滤波）
 *   - wcz_hei_fus  -- 垂直高度融合（微分修正滤波）
 *
 * 融合原理:
 *   加速度积分提供高频响应（短期准确），气压计/ToF 提供低频基准（长期稳定），
 *   互补滤波器自动分配信任权重，输出平滑且无漂移的高度和速度估计。
 *
 * 架构位置:
 *   由 FlightDataCal.c 的 WCZ_Fus_Task() 调用，输出供 AltCtrl 使用。
 *
 * 教学提示:
 *   - wcz = World Coordinate Z axis（世界坐标 Z 轴，即垂直方向）
 *   - 融合滤波器的核心结构定义在 Filter.h（_inte_fix_filter_st）
 *   - BARO_SPEED_ERR_LIM 限制融合修正的最大速度偏差，防止突变
 */
#include "MotionCal.h"
#include "Math.h"
