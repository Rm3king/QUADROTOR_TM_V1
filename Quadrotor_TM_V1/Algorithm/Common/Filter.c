 ...
 * 功能描述:
 *   飞控滤波器工具库，提供以下滤波算法：
 *   1) inte_fix_filter()       —— 积分修正滤波（高度融合核心）
 *   2) fix_inte_filter()       —— 微分修正滤波
 *   3) limit_filter/_2/_3()    —— 限幅低通滤波（抑制噪声同时保留信号幅度）
 *   4) LPF_1_() 宏             —— 一阶低通滤波（最常用，内联宏实现）
 *   5) Moving_Average()        —— 滑动平均滤波
 *   6) step_filter()           —— 步进限幅滤波（限制变化率）
 *   7) jyoun_filter()          —— 自适应低通滤波
 *   8) h2w/w2h_2d_trans()      —— 航向↔世界 2D 坐标变换
 *
 * 架构位置:
 *   纯计算函数库，被控制层和数据融合层广泛调用。
 *
 * 教学提示:
 *   - LPF_1_ 是离散化一阶 RC 低通，截止频率 hz，采样周期 t
 *   - inte_fix_filter 本质是互补滤波器：用观测量修正积分量的漂移
 *   - limit_filter 系列用于既要低通又要保持信号幅度的场景
 */
#include "Filter.h"
#include "Math.h"
...
void limit_filter(float T,float hz,_lf_st *data,float in) //增量滤波，适合大噪声低滞后（无缝），收敛最快
{
	float abs_t;
...
void limit_filter_2(float T,float hz,_lf_st *data,float in) //增量滤波，均衡，但滞后稍大(数据反向等待低通反向)
{
	float abs_t;
	LPF_1_(hz,T,	 in,(data->lpf_1)); 
	abs_t = ABS(data->lpf_1);
...
void limit_filter_3(float T,float hz,_lf_st *data,float in) //增量滤波，适合低噪声滞后较小(等幅大噪声平行不收敛),噪声大收敛过慢
{
	float abs_t;
	LPF_1_(hz,T,	 in,(data->lpf_1)); 
	abs_t = ABS(in);
...
//#include "math.h"
//void init_low_pass2_fliter(float sample_freq, float cutoff_freq,_filter_st* imu_filter)
//{
...
//float low_pass2_filter(float sample,_filter_st* imu_filter)//低通二阶滤波器
//{
