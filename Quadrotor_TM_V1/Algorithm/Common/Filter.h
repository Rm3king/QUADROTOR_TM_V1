...
void limit_filter(float T, float hz, _lf_st *data, float in);
void limit_filter_2(float T, float hz, _lf_st *data, float in);
void limit_filter_3(float T, float hz, _lf_st *data, float in);
void init_low_pass2_fliter(float sample_freq, float cutoff_freq, _filter_st *imu_filter);
float low_pass2_filter(float sample, _filter_st *imu_filter);
void steepest_descend(s32 arr[], u8 len, _steepest_st *steepest, u8 step_num, s32 in);
void jyoun_limit_deadzone_filter(float T, float hz1, float hz2, _jldf_st *data, float in);
