 ...
 *
 * 功能描述:
 *   飞控数学工具库，提供嵌入式环境下的高效数学运算：
 *   1) my_sin()/my_cos()  -- 查表快速三角函数（256 点 atan 表）
 *   2) my_sqrt()          -- 快速平方根（牛顿迭代）
 *   3) safe_div()         -- 安全除法（除零保护）
 *   4) my_atan2_deg()     -- 查表快速 atan2（输出角度）
 *   5) my_deadzone()      -- 死区函数（摇杆中位消抖）
 *   6) length_limit()     -- 二维向量模长限幅
 *   7) h2w/w2h_2d_trans() -- 航向与世界坐标 2D 旋转变换
 *
 * 架构位置:
 *   纯计算函数库，无硬件依赖，被全项目广泛调用。
 *
 * 教学提示:
 *   - 嵌入式系统中避免使用标准库 sin/cos/sqrt，浮点运算开销大
 *   - 查表法用空间换时间，精度足够飞控使用
 *   - safe_div 在飞控中至关重要，除零会导致 NaN 传播使飞机失控
 */
#include "Math.h"
