/*
 * 文件名称: FlyCtrl.c
 * 所属模块: Application / Mission
 *
 * 功能描述:
 *   程控飞行模块，处理上位机下发的飞行指令序列：
 *   起飞、降落、前后左右移动、偏航旋转、定高悬停等。
 *   将上位机指令翻译为速度期望和姿态期望，供控制层执行。
 *
 * 架构位置:
 *   由 Scheduler 按 6ms 周期调用。
 *   输出 program_ctrl 结构体被 FlightCtrl / AttCtrl 读取。
 *
 * 教学提示:
 *   - 这是实现航点飞行、编程表演的基础模块
 *   - 每个命令通过 val(距离/角度) 和 spd(速度) 参数化控制
 */
#include "FlyCtrl.h"
#include "DT.h"
#include "FcData.h"
