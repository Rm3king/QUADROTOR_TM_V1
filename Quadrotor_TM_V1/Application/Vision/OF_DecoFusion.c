...
/*
 * 文件名称: OF_DecoFusion.c
 * 所属模块: Application / Vision
 *
 * 功能描述:
 *   光流数据解耦与融合模块，将原始光流像素位移转换为地面速度估计：
 *   1) ANO_OF_Data_Get()  -- 解析光流原始数据包，提取位移和高度
 *   2) ANO_OF_Decouple()  -- 姿态解耦：去除无人机自身旋转对光流的影响
 *   3) OF_INS_Get()       -- 惯性导航估计：用加速度积分估算速度
 *   4) ANO_OF_Fusion()    -- 互补融合：光流观测修正惯性积分漂移
 *   5) OF_State()         -- 光流数据有效性判定
 *
 * 解耦原理:
 *   光流传感器输出的像素位移 = 平移分量 + 旋转分量
 *   通过 IMU 陀螺仪测量的角速度减去旋转分量，提取纯平移速度
 *
 * 融合结构:
 *   光流速度观测（低漂移但有噪声）+ 加速度积分（平滑但有漂移）
 *   → 互补滤波 → 地面速度估计 of_rdf.gnd_vel_est_h[]
 *
 * 架构位置:
 *   由 Scheduler 按 11ms 周期调用。
 *   输出供 LocCtrl.c 作为水平速度反馈。
 *
 * 教学提示:
 *   - 光流解耦是使用光流定点的关键，不解耦会导致转弯时漂移
 *   - OF_VALID_FLAG (0xF5) 是光流模块数据有效的标记字节
 *   - 融合增益 of_fus_err_i 是长期漂移修正，响应较慢但稳定
 */
...
static u8 of_buf_update_flag;
_of_data_st of_data;
_of_rdf_st of_rdf;
float of_rot_d_degs[2];
static float of_fus_err[2],of_fus_err_i[2];

#define OF_VALID_FLAG          0xF5
...
static void ANO_OF_Data_Get(float *dT_s,u8 *of_data_buf)
{
    ...
    if(of_buf_update_flag != BUF_UPDATE_CNT)
    {
        ...
        if(ADD == of_data_buf[18])
        {
            ...
            if(of_data.valid != OF_VALID_FLAG)
            {
