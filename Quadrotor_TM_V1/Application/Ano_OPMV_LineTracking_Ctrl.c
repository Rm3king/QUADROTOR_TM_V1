...
//				if(opmv.lt.sta == 2)
		...
		{
			//刹车减速
			ano_opmv_lt_ctrl.exp_velocity_h_cmps[0] = 20;//识别到直角后减速前进的速度cm/s
			ano_opmv_lt_ctrl.exp_velocity_h_cmps[1] = 0;//左右纠正速度复位
			//持续时间，转弯后偏内测，则加长时间;反之减少时间
			if(elapsed_time_ms<1500)
			{
...
//				if(opmv.lt.sta == 3)
		...
		{
			//
			ano_opmv_lt_ctrl.exp_velocity_h_cmps[0] = 25;//边旋转边给前进速度，走出一定圆弧
			ano_opmv_lt_ctrl.exp_velocity_h_cmps[1] = 0;//左右纠正速度复位
...
void ANO_LTracking_Ctrl(u8 *dT_ms,u8 en)
{
	...
	if(en)
	{
		...
		{
			//
			ano_opmv_lt_ctrl.exp_yaw_pal_dps = -opmv.lt.angle *0.5;
